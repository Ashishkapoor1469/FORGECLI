...
async function handleModelSelection() {
    ...
    if (selection === "delete") {
        ...
        if (prompt.isCancel(delTarget) || delTarget === -1)
            return;
        const delIdx = delTarget;
...
async function handleAdminMenu() {
    ...
    while (true) {
        ...
        if (action === "back" || prompt.isCancel(action))
            break;
        if (action === "view_memory") {
            const mems = memory.getMemories();
            prompt.log.info(chalk_1.default.dim(JSON.stringify(mems, null, 2)));
        }
            ...
            prompt.log.info(chalk_1.default.dim(JSON.stringify(state, null, 2)));
        }
...
async function main() {
    ...
    const workspaceStr = config.activeProject ? ` | Workspace: ${chalk_1.default.green(config.activeProject)}` : "";
    prompt.log.info(`Model: ${chalk_1.default.cyan(`${config.provider}/${config.model}`)}${workspaceStr}${buddyStr} | Type ${chalk_1.default.bold("/help")} for commands`);
    while (true) {
        ...
        if (input.startsWith("/cd ")) {
            ...
            const targetPath = (0, path_1.join)(process.cwd(), "workspace", folder);
            if (!(0, fs_1.existsSync)(targetPath)) {
                ...
                const wsStr = ` | Workspace: ${chalk_1.default.green(config.activeProject)}`;
                const bdStr = b ? ` | Buddy: ${chalk_1.default.magenta(b)}` : "";
                prompt.log.info(`Model: ${chalk_1.default.cyan(`${config.provider}/${config.model}`)}${wsStr}${bdStr} | Type ${chalk_1.default.bold("/help")} for commands`);
                prompt.log.success(`Switched active workspace to ${chalk_1.default.cyan(`workspace/${folder}`)}`);
            }
            continue;
        }
        if (input === "/clear") {
            ...
            const wsStr = config.activeProject ? ` | Workspace: ${chalk_1.default.green(config.activeProject)}` : "";
            const bdStr = activeBuddy ? ` | Buddy: ${chalk_1.default.magenta(activeBuddy)}` : "";
            prompt.log.info(`Model: ${chalk_1.default.cyan(`${config.provider}/${config.model}`)}${wsStr}${bdStr} | Type ${chalk_1.default.bold("/help")} for commands`);
            continue;
        }
        if (input === "/memory") {
