...
class WorkerAgent {
    ...
    async execute(task, taskDescription, context, config, globalObjective, projectName, fileOutput, directoryManifest, existingCode, attempt = 1, maxRetries = 3) {
...
6. Generate REAL, COMPLETE, WORKING code — not placeholders or stubs.`;
        try {
            ...
            if (attempt < maxRetries) {
                return this.execute(task, taskDescription, context, config, globalObjective, projectName, fileOutput, directoryManifest, existingCode, attempt + 1, maxRetries);
            }
            return {
                task_id: task.task_id,
                agent: task.agent,
